# APP 内容接口文档（政策 / FAQ / 反馈）

- status: approved
- owner: <owner>
- last_updated: 2026-02-14
- source: `doc/app-content-api.md`
- version: v1.0.0

本文档面向 APP/小程序端开发，描述 `/api/v1/app/content/*` 相关接口（单页内容、FAQ、用户反馈）及反馈图片上传接口。
...
### 1.2 请求头
- `X-TenantID`：租户 ID（强烈建议传；不传时服务端会默认使用第 1 个租户）
...
- 服务端返回字段 `lang` 会规范化为 `zh-CN` 或 `en-US`
...
渲染建议：
- APP 端建议使用 `content_html` + `rich-text` 渲染。
- `content_markdown` 主要用于调试或兜底。

## 3. FAQ（常见问题）

FAQ 不分类，支持置顶与排序（默认按 `is_pinned DESC, sort DESC, updated_at DESC`）。
...
渲染建议：列表页只展示 `question`，详情展开使用 `answer_html`（或 `answer_markdown` 兜底）。
...
说明：
- 提交反馈时在 `images` 直接传该 `path` 列表。
- 展示图片可按约定拼接：`{SERVER}` + `path.slice(1)`。
...
Query：
- `appid` string 可选：应用 AppID（不传返回当前用户全部反馈）
- `page` int 必填
- `page_size` int 必填

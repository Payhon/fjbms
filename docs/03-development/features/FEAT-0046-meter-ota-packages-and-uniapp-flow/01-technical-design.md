...
- 仪表设备：
  ...
  - 仪表 OTA 的 `0x53` 写包等待时间为 45s，避免 Bootloader 在 4KB/8KB 等页/扇区边界擦写时被默认超时提前打断；Android 仪表升级采用 ACK 驱动、带响应写、100ms 最小帧间隔和 300ms 的 4KB 边界等待，发生包超时后自动切换到 100ms 包间等待与 1500ms 边界等待。蓝牙透传模块按单次 BLE 写入透传 Boot 帧，不能把单个 `0x53` Boot 帧拆成多个 20 字节 BLE 写入。
  ...
  - BLE OTA 在进入 Boot 流程前开启独占通讯会话：取消正在等待的普通状态读取，最多等待 1.2s 清空既有串行队列；升级期间拒绝新的非 Boot 请求，结束后恢复普通轮询，避免 `0x50` 被旧状态请求阻塞约 11s。该队列隔离同时作用于蓝牙 BMS 与蓝牙仪表。
  - 蓝牙 BMS 与蓝牙仪表的 `0x54` 使用单个 5s 严格应答窗口，窗口内仍按 300ms/600ms/900ms 补发；不再启动三轮 finalize 或 iOS 替代写模式重试，也不接受残留 `0x53` ACK 作为完成应答。只有收到命令为 `0x54` 且 `status=0` 的响应才成功。
  - 仪表 OTA 在上述通用蓝牙边界策略外，继续保留专用协议与时序：`0x50` 广播查询、跳过 `0x51`、`0x52` 固定 9600，以及 Android 仪表专用 ACK 驱动和超时降速保护；BMS OTA 不使用这些仪表专用时序。

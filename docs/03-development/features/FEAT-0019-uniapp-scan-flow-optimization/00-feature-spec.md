...
- 目标：
  ...
  8. APP 端扫码添加入口支持从系统相册选择二维码图片识别，能力与微信小程序扫码入口保持一致。
...
- APP 端 `scanCode` 不限制 `onlyFromCamera`，允许用户通过摄像头或系统相册二维码图片完成同一套扫码添加路由。
...
15. APP 端点击扫码入口时，除摄像头实时扫码外，也可从系统相册选择二维码图片；相册识别结果必须复用现有 MAC/UUID 解析、已绑定设备直达和添加分流规则。
...
- APP 端相册二维码识别依赖系统扫码组件能力；真机验收需同时覆盖 iOS/Android 的相册权限提示与识别入口展示。

...
- UniApp 蓝牙扫描页不再隐藏已存在于“我的设备”的扫描结果；命中已绑定 `ble_mac` 的设备卡片会保留在列表中，并展示“该设备已添加到我的设备”备注。
- UniApp 蓝牙扫描页点击已标记“该设备已添加到我的设备”的设备时，现会直接进入该设备详情页，不再重复进入添加向导。
- UniApp 添加/绑定接口错误提示现会优先展示后端 `data.message` 详情，便于用户和现场调试区分具体失败原因。
- UniApp APP 端扫码添加入口现支持从系统相册选择二维码图片识别；摄像头扫码和相册识别共用同一套已绑定设备直达、BMS 添加、仪表临时会话与 UUID 绑定分流逻辑。
- UniApp 扫码入口文案已调整为“扫码/相册识别 / Scan or Album”，iOS 相册权限说明同步补充设备二维码图片用途。
...
- UniApp 设备详情页复用 warm BLE 会话前会先做轻量健康探测；若缓存连接已失效，会自动断开并重连，避免页面停在“等待首帧数据”。
- UniApp 设备详情页在普通蓝牙 BMS 首帧读取失败时会自动重连恢复，连续失败后展示“重新连接读取 / 重新读取”入口，用户不再需要退出页面再进入。
- UniApp 设备详情页 BLE 常规状态轮询频率已由 `5s` 提升到 `2s`，缩短详情面板状态刷新延迟。
...
- 本次新增的 handoff、BLE 会话复用与首帧恢复均为客户端内存/本地存储级逻辑，无额外服务端升级步骤。
...
## 5. 已知问题
- 尚未完成本轮真机蓝牙联调闭环，需重点验证 iOS App 端蓝牙扫描启动稳定性、从扫描页进入添加向导的连接启动与服务树发现稳定性、连接成功后 `post-connect warmup` 是否足以覆盖设备首包准备时间、已连接设备扫码直达详情时是否稳定接管现有 BLE 会话、warm BLE 探测失败后是否能可靠重连、首页进详情首帧失败后是否自动恢复并显示人工重试入口、remembered `deviceId` 是否能稳定用于 iPhone 详情页直连、直连失败后的短扫回退是否及时命中、请求期内 BLE 写入/探测 API 是否仍会出现无回调、BLE 扫描点击仪表设备后的临时会话首连体验、二次扫码写目标地址时序，以及 iOS/Android APP 端相册二维码识别入口与权限提示。

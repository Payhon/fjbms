...
- 设备详情仪表页顶部状态入口按只读状态寄存器判断：
  - 故障：`0x131` 失效状态；
  - 告警：`0x134~0x135` 告警状态；
  - 保护：`0x12F~0x130` 保护状态。
- `0x4xx` 为读写配置项，只影响阈值、延时等配置，不参与故障/告警/保护入口显示判断。
...
- UniApp 与 Frontend Web `BmsClient.readAllStatus()` 不再依赖一次性连续读取整个状态区，而是按协议感知拆成两段：
  - 第一段固定读取 `0x100~0x134`，数量为 `0x35`（53）个寄存器；
  - 额外尝试读取 `0x135`，用于补齐 `0x134~0x135` 告警状态 32BIT 的高 16 位；
  - 第二段固定从 `0x141` 读取到动态状态区末尾；
  - 中间未读取的 `0x136~0x140` 缺口以零值回填，若旧板不支持 `0x135` 则 `0x135` 也按零值回填，供 `parseStatusRegisters()` 继续按连续地址视图解析。
...
- Frontend：
  ...
  - `frontend/src/common/lib/bms-protocol/client.ts`
...
- 组包方式：
  ...
  - 单独尝试读取 `0x135` 作为告警状态高 16 位；
  - 第二次读取 `0x141~lastAddr`；
  - 在内存中构造从 `0x100` 开始的连续 `Uint16Array`；
  - `0x136~0x140` 使用默认 `0x0000` 占位，不对外暴露新的兼容类型；
  - 若 `0x135` 单寄存器读取失败，则仅该高位告警字按默认值处理，整体状态读取继续走旧板兼容链路。
- 这样既可以绕过旧板未实现的中间寄存器，又能在新板支持 `0x135` 时完整解析 `0x134~0x135` 告警状态 32BIT。

## 4. UI 方案
- 保留仪表盘顶部故障/告警/保护提示入口，用于快速查看当前激活项。
- 顶部入口显示规则：
  - “故障”只统计 `status.failureStatus` 中为 `true` 的位；
  - “告警”只统计 `status.alarmStatus` 中为 `true` 的位；
  - “保护”只统计 `status.protectionStatus` 中为 `true` 的位；
  - 不读取参数设置页 `0x4xx` 配置项，也不把保护状态并入故障入口。
...
- UniApp 与 Frontend Web 协议层重点验证：
  - 第一次读取数量为 `0x35`（53）个寄存器，不跨过 `0x135` 连续读取；
  - `0x135` 单寄存器读取成功时可补齐告警高 16 位；
  - `0x135` 单寄存器读取失败时不影响旧板兼容状态轮询；
  - `0x135` 可选读取结果与 `0x136~0x140` 回填后，`parseStatusRegisters()` 不抛地址越界异常；
  - `0x141` 之后的动态区身份信息、电芯电压与温度解析保持不变。
- Frontend 执行 `pnpm typecheck`，确保 Web 端面板和协议类型改动可编译。
- 手工验收关注：
  - `0x4xx` 配置项开启但 `0x1xx` 状态位未触发时，顶部故障/告警/保护入口不显示。
  - `0x131`、`0x134~0x135`、`0x12F~0x130` 分别触发时，只显示对应入口。
  - 有保护项激活时的摘要数量和列表状态；

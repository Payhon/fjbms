...
3. 在线状态兜底修复
   ...
   - 已调整 `TelemetryUplink`、`AttributeUplink`、`EventUplink`：收到业务数据后始终执行自动上线；若设备未配置显式心跳，则刷新默认保活 TTL（`heartbeat.default_online_ttl_sec`，默认 300 秒），确保“有数据上报即在线”，覆盖设备约 150 秒一次上报的场景。
